...
{
    public const int MAIN_UI_ID = 0, LOBBY_UI_ID = 1, PICKUP_UI_ID = 2, ACHIVEMENT_UI_ID = 3, CARDSTORE_UI_ID = 4;
    public static int currUIPanelNumber = 0;
    public static int prevUIPanelNumber = 0;
    public static int currSceneNumber = 0;// 0 = map, 1 = character, 2 = car
    public static int currStoreNumber = 0;// 0 = cardStore, 1 = upgrade
    public static string storeBackButtonSceneName = "CarPickUp";
    public static Texture myTipTexture = null;
    ...
    public static bool firstClearNightmare = false;
    ...
    public static float pickUpBGMTime = 0;
    public static float storeBGMTime = 0;
    ...
    public static int nightmareStageNum = -1;
    public static bool playingNightmare = false;
    ...
    {
        Debug.Log("previous progress = " + saveData["progress"].n + "; new progress = " + progress);
        // set TotalCoins number
        float newProgress = Mathf.Max(progress, saveData["progress"].n);
        saveData.SetField("progress", newProgress);

        Debug.Log("new progress = " + saveData["progress"].n);
    }
    ...
    {
        //Debug.Log("GetCurrentCharAttribute char id = " + charIndex + " characterData.GetField(\"characters\").list length = "+ characterData.GetField("characters").list.Count);
        ...
        float levelDiff = GetCurrentCharLevel(charIndex) - 1;
        if (lv != -1)
        {
            levelDiff = lv - 1;
        }
        ...
        for (int i = 0; i < carData.GetField("cars").list[carIndex].GetField("slotAttributes").list.Count; ++i)
        {
            ...
            if (slotAttr.Equals("ATK"))
            {
                ...
                if (cardID != -1)
                {
                    ...
                    else if (cardName.StartsWith("trade"))
                    {
                        hpModifier -= cardData.GetField("ATK").list[cardID].GetField("attributes").list[0].n;
                        attackModifier += cardData.GetField("ATK").list[cardID].GetField("attributes").list[1].n;
                    }
                }
            }
    ...
    public static void SetCurrentCardInSlot(int carIndex, int slotNumber, int cardID)
    {
        ...
        string attribute = carData.GetField("cars").list[carIndex].GetField("slotAttributes").list[slotNumber].str;
        if (oldCardIdx == cardID)
        {
            return;
        }
        
        if ( saveData.GetField("cards").GetField(attribute).list[cardID].GetField("number").n <= 0)
        {
            // no available cards
            return;
        }
    ...
    public static JSONObject GetCarAttribute(int carID, int lv = -1)
    {
        /*
         * return type:
         * {
        ...
        ret.AddField("unlocked", GetProgress() >= carData.GetField("cars").list[carID].GetField("unlocked").n);
        ret.AddField("level", saveData.GetField("cars").list[carID].GetField("level").n);
        float lvlDiff = saveData.GetField("cars").list[carID].GetField("level").n - 1;
        if (lv != -1)
        {
            lvlDiff = lv - 1;
        }

        float hp = carData.GetField("cars").list[carID].GetField("baseStatus").GetField("hp").n + lvlDiff * carData.GetField("cars").list[carID].GetField("statusModifier").GetField("hp").n;
        ret.AddField("hp", hp);
        float mp = carData.GetField("cars").list[carID].GetField("baseStatus").GetField("mp").n + lvlDiff * carData.GetField("cars").list[carID].GetField("statusModifier").GetField("mp").n;
        ret.AddField("mp", mp);
        float speed = carData.GetField("cars").list[carID].GetField("baseStatus").GetField("speed").n + lvlDiff * carData.GetField("cars").list[carID].GetField("statusModifier").GetField("speed").n;
        ret.AddField("speed", speed);
        float attack = carData.GetField("cars").list[carID].GetField("baseStatus").GetField("attack").n + lvlDiff * carData.GetField("cars").list[carID].GetField("statusModifier").GetField("attack").n;
        ret.AddField("attack", attack);
        float CD = carData.GetField("cars").list[carID].GetField("baseStatus").GetField("CD").n + lvlDiff * carData.GetField("cars").list[carID].GetField("statusModifier").GetField("CD").n;
        ret.AddField("CD", CD);
        float defense = carData.GetField("cars").list[carID].GetField("baseStatus").GetField("defense").n + lvlDiff * carData.GetField("cars").list[carID].GetField("statusModifier").GetField("defense").n;
        ret.AddField("defense", defense);
        return ret;
    }

    public static JSONObject GetCharacterAttribute(int charID,int lv = -1)
    {
        /*
         * return type:
         * {
    ...
    private static float ExpToNext(int currLv, float currExp)
    {
        if(currLv == (int)(characterData.GetField("maxLevel").n) )
        {
            return - 1;
        }

        float nextExpTotal = characterData["baseExp"].n * Mathf.Pow(characterData["expModifier"].n, currLv-1);
        return nextExpTotal - currExp;
    }
    public static void SetExp(int charID,float exp)//add exp points
    {
        int lv = (int)saveData["characters"][charID]["level"].n;
        float e = saveData["characters"][charID]["exp"].n;
        float expNext = ExpToNext(lv, e);
        while (exp >= expNext)
        {
            exp -= expNext;
            lv++;
            e = 0;
            expNext = ExpToNext(lv, e);
        }

        float currLv = lv, currExp = exp;
        saveData["characters"][charID].SetField("exp", currExp);
        saveData["characters"][charID].SetField("level", currLv);

    }
    public static string NumToTimeStr(float n)
    {
        if (n == -1)
        {
            return "--";
        }
        int lastFour = (int)(Mathf.Round(n * 100)) % 6000;
        string seconds = ((lastFour % 10000) / 1000).ToString() + ((lastFour%1000) / 100).ToString() + "." + ((lastFour % 100) / 10).ToString() + ((lastFour % 10)).ToString();
        string minStr = ((int)(n / 60)).ToString();
