...
public class ItemCollector : MonoBehaviour {
	...
	void OnTriggerEnter(Collider other) 
	{
        ...
        {
            if (GetComponent<Gluttony>())
            {
                GetComponent<Gluttony>().Glutton();
            }
            ...
            else if (other.GetComponent<HealthPotion>() != null)
            {
                float hpAmount = other.GetComponent<HealthPotion>().healthRatio * GetComponent<CarStatus>().getMaxHP();
                GetComponent<CarStatus>().increaseHP(hpAmount);
            }
            else
            {
    ...
    public void useItem()
    {
        ...
        if (itemIdx != -1)
        {
            ...
            else //traps
            {
                ...
                if (StaticVariables.playingNightmare && GetComponent<AIScript>() != null)
                {
                    firstCar = FindObjectOfType<PlayerController>().GetComponent<Car>();
                }
