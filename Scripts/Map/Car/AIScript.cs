...
public class AIScript : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
        ...
        if (flag == 0)
        {
            //Debug.Log("newSteerFactor = " + newSteerFactor);
    ...
    {
        if (Physics.Raycast(pos, dir, out hit, length) && (hit.transform.GetComponent<wallHeightAdjust>()==null) && (!hit.transform.GetComponent<Collider>().isTrigger || (hit.transform.GetComponent<TrapWeapons>() != null && hit.transform.GetComponent<TrapWeapons>().attacker!=null)) )
        {
            return true;
        }
        return false;
