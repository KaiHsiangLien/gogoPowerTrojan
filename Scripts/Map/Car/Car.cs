...
public class Car : MonoBehaviour
{
    ...
    public const float RUNNING_ANGULAR_DRAG = 2f;
    public float angularDragModifier = 1;
  ...
  /*  // Use this for initialization 
    ...
    */
    ...
    {
        return !stoppedBySkill && !stunned && !stopFlag;
    }
    public bool ableToUseSkill()
    {
        if (!mySkill.ableToActivate())
        {
            return false;
        }
    ...
    public void startRunning()
    {
        if (!stunned)
        {
            foreach (Rigidbody rb in GetComponentsInChildren<Rigidbody>())
            {
                ...
                if (transform.rotation.eulerAngles.x > maxTiltAngle / 2 && transform.rotation.eulerAngles.x < (360 - maxTiltAngle / 2))
                {
                    Debug.Log(name + " is tilting!! Time = "+Time.time);
                    rb.angularDrag = RUNNING_ANGULAR_DRAG * angularDragModifier;
                }
                else if (transform.rotation.eulerAngles.z > maxTiltAngle / 2 && transform.rotation.eulerAngles.z < (360 - maxTiltAngle / 2))
                {
                    Debug.Log(name + " is tilting!! Time = " + Time.time);
                    rb.angularDrag = RUNNING_ANGULAR_DRAG * angularDragModifier;
                }
